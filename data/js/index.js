...
$(document).ready(function () {
  ...
  $('#sendButton').click(function () {
    ...
    var regex = /^[Rr][1-9A-HJ-NP-Za-km-z]{26,33}$/;
    var validAddress = true;
    if (!regex.test(String(address))) {
      validAddress = false;
    } else {
      try {
        new bitcoinjs_avn.address.fromBase58Check(address);
      } catch (e) {
        validAddress = false;
      }
    }
    ...
    wallet.send(address, val, FEE, password).then(
      function (txid) {
        ...
        var text =
          ...
          '<a href="https://explorer.avn.network/tx/?txid=' +
          txid +
          '" target="_blank">' +
          txid +
  ...
  $('#setUnits').click(function () {
    preferences.getBTCUnits().then(function (units) {
      var availableUnits = ['AVN', 'mAVN', 'µAVN'];
      var tableBody = '<tr>';
  ...
  $('#importPrivateKeyConfirm').click(function () {
    ...
    try {
      new bitcoinjs_avn.ECPair.fromWIF(privateKey);
    } catch (e) {
  ...
  /*
   * About version and version under avian image
   */
