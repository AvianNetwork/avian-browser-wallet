...
$(document).ready(function () {
    ...
    $('body').on('click', 'a', function (e) {
        ...
        if (/^avian:[Aa][1-9A-HJ-NP-Za-km-z]{26,33}/.test(href)) {
            var addresses = href.match(/[Aa][1-9A-HJ-NP-Za-km-z]{26,34}/);
            var address = null;
            if (addresses) {
                address = addresses[0];
            }
    ...
    function showPopup(address, amount, rect) {
        util.iframe('paypopup.html').then(function (iframe) {
            ...
            if (!address || !/^[Aa][1-9A-HJ-NP-Za-km-z]{26,34}$/.test(String(address))) {
                address = null;
            } else {
                try {
            ...
            $iframe.find('#button').click(function () {
                ...
                if (!address) {
                    ...
                    if (!/^[Aa][1-9A-HJ-NP-Za-km-z]{26,34}$/.test(String(newAddress))) {
                        validAddress = false;
                    } else {
                        try {
                            new bitcoin.Address.fromBase58Check(address);
